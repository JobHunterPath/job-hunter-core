...
_MAX_SNIPPET_CHARS = 2000  # snippet chars fed to the LLM for validation/repair
...
def validate(
    ...
    *,
    ...
    def _validate_job(args: tuple[int, dict]) -> None:
        ...
        snippet = (job.get("snippet") or "")[:_MAX_SNIPPET_CHARS]
        if not snippet:
            with results_lock:
        ...
        if deterministic_reason and not bypass_max_years:
            ...
            with results_lock:
                results.append(
                    (idx_orig, "rejected", {**job, "_rejection_reason": deterministic_reason})
                )
            return
        if deterministic_reason and bypass_max_years:
            logger.info(
                f"{prefix}: strategic override bypasses experience limit: {deterministic_reason}"
            )
        ...
        try:
            ...
            try:
                ...
                repaired = get_llm_client("validation").complete(
                    system=_SYSTEM,
                    user=_REPAIR_PROMPT.format(raw=raw[:_MAX_SNIPPET_CHARS]),
                    model=settings.model,
                    max_tokens=settings.max_tokens,
                    response_format="json",
                )
