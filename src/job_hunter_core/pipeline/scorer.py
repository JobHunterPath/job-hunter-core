...
from job_hunter_core.core.config import ROOT, load_api_config, profile_path
...
_MAX_REPAIR_INPUT_CHARS = 2000  # raw LLM response chars passed to the repair prompt

with open(profile_path("resume_tex", "resume.tex"), encoding="utf-8") as f:
    BASE_RESUME = f.read()


def load_scoring_config() -> dict[str, object]:
...
def _compact_latex_resume(tex: str) -> str:
    ...
    text = re.sub(
        r"\\(documentclass|usepackage|geometry|hypersetup)(?:\[[^\]]*\])?\{[^}]*\}", " ", text
    )
    ...
    try:
        ...
        try:
            ...
            repaired = get_llm_client("scoring").complete(
                system=system,
                user=REPAIR_PROMPT.format(raw=raw[:_MAX_REPAIR_INPUT_CHARS]),
                model=settings.model,
                max_tokens=settings.max_tokens,
                response_format="json",
                cache_system=True,
...
def strategic_override_companies(config: dict) -> list[str]:
    ...
    return [
        ...
        if str(override.get("company", "")).strip()
        and override.get("bypass_max_years_experience", True)
    ]
