...
def register_company(job: dict, root: str | Path, region: str | None = None) -> None:
    """Add the job's company to the active enabled search_config.yml region."""
    ...
    if not region:
        logger.debug("[register] No active region on job %s - skipping", company_name)
        return

    region_config = sc_data.get("regions", {}).get(region)
    if not region_config or not region_config.get("enabled", True):
        logger.debug(
            "[register] Region %r is missing or disabled - skipping %s", region, company_name
        )
        return
    ...
    if company_name.lower() not in existing_names:
        ...
        logger.info(
            "[register] Added %s (%s) -> search_config.yml region: %s",
            company_name,
            career_url,
            region,
        )
    else:
        logger.debug("[register] %s already in region %s", company_name, region)
