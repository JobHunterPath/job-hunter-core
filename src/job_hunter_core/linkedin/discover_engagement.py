...
from job_hunter_core.core.config import setup_logging
from job_hunter_core.linkedin.common import (
    append_section,
    complete_linkedin,
    configured_path,
    extract_json,
...
from job_hunter_core.sources.search_providers import search_web
...
TARGET JOB TITLES FROM SEARCH CONFIG:
{job_titles}

TARGET REGIONS FROM SEARCH CONFIG:
{regions}

TARGET COMPANIES FROM SEARCH CONFIG:
{companies}
...
def _search_context(policy: dict[str, Any]) -> dict[str, list[str]]:
    data = yaml.safe_load(read_text(repo_path("config/search_config.yml"), "{}")) or {}
    regions: list[str] = []
    companies: list[str] = []
    titles = list(data.get("global_search", {}).get("job_titles", []) or [])
    for region_name, region in (data.get("regions", {}) or {}).items():
        ...
        for title in region.get("job_titles", []) or []:
            if title not in titles:
                titles.append(title)
        for company in region.get("companies", []) or []:
...
def _fallback_strategy(
    ...
    people = list(dict.fromkeys(audience + titles + pillars))[
        : _setting(policy, "strategy", "people_query_count", 5)
    ]
    templates = strategy_policy.get("fallback_recruiter_query_templates", []) or []
    recruiter_queries = [
        str(template).format(title=title)
        for title in titles
    ...
    return {
        ...
        "recruiter_queries": recruiter_queries[
            : _setting(policy, "strategy", "recruiter_query_count", 5)
        ]
        or ["technical recruiter"],
        "target_companies": context.get("companies", [])[
            : _setting(policy, "strategy", "target_company_count", 8)
        ],
    }
...
def _search_strategy(config: dict[str, Any]) -> dict[str, list[str]]:
    ...
    try:
        ...
        logger.warning(
            "[linkedin] Search strategy generation failed; using config-derived fallback: %s", exc
        )
        return fallback
...
def _collect_public_results(
    ...
    for kind, query in _queries(config, strategy, policy):
...
def _score_candidate(
    ...
    if len(candidate.description.split()) >= _setting(
        policy, "scoring", "enough_evidence_words", 18
    ):
...
def _dedupe_and_select(
    ...
    return {
        "people": sorted(people, key=lambda c: c.score, reverse=True)[
            : int(caps.get("max_people_to_rank", 5))
        ],
...
def _generate_suggestions(
    ...
    try:
        ...
        logger.warning(
            "[linkedin] Discovery generation failed; using deterministic drafts: %s", exc
        )
        payload = {}
    ...
    for candidate in all_people:
        ...
        candidate.message_variants = [
            _trim_words(
                str(message).strip(), int(config.get("networking", {}).get("max_message_words", 70))
            )
...
def _update_state(state: dict[str, Any], selected: dict[str, list[Candidate]]) -> dict[str, Any]:
    for candidate in selected["people"] + selected["recruiters"]:
        state.setdefault("seen_people", []).append(
            _stable_key(candidate.url, candidate.fingerprint)
        )
