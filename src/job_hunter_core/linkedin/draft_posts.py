...
from job_hunter_core.core.config import ROOT, setup_logging
...
def draft(config_path: Path | None = None) -> list[Path]:
    ...
    prompt = PROMPT.format(
        ...
        confidentiality=format_yaml_list(
            config.get("confidentiality", {}).get("forbidden_public_details", [])
        ),
...
"""
    ...
    if created and draft_cfg.get("mark_converted", True):
        converted_ids = [
            str(item.get("idea_id") or selected[i]["id"])
            for i, item in enumerate(payload[: len(created)])
        ]
