...
Set affid in config/api_config.yml under http.job_boards.careerjet.affid.
...
class CareerjetSource(JobSourceAdapter):
    ...
    def fetch(
        ...
        *,
        ...
        source_cfg = (
            load_api_config().get("http", {}).get("job_boards", {}).get("careerjet", {}) or {}
        )
            ...
            for title in title_filters:
                for page in range(1, max_pages + 1):
                    ...
                    try:
                        ...
                        logger.warning(
                            "[careerjet] request failed for %r in %s: %s", title, region_name, exc
                        )
                        break
