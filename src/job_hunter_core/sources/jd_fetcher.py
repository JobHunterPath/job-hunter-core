...
from job_hunter_core.core.utils import strip_html
...
def _fetch_greenhouse_api(
    ...
    if data:
        job = _greenhouse_job_from_data(
            data, url, company, max_description_chars, title_min_chars, title_max_chars
        )
    ...
    if item:
        return _greenhouse_job_from_data(
            item, url, company, max_description_chars, title_min_chars, title_max_chars
        )
    return None
...
def _fetch_workable_api(
    ...
    location = (
        location_obj.get("location", "") if isinstance(location_obj, dict) else str(location_obj)
    )
    if not content and not title:
        return None
    company_name = company.replace("-", " ").replace("_", " ").title()
...
def fetch_jd(url: str, use_llm: bool = True, *, expected_title: str = "") -> dict | None:
    ...
    try:
        ...
        cfg_raw = load_api_config().get("http", {}).get("jd_fetcher", {}) or {}
    except Exception:
        pass
    ...
    if is_greenhouse:
        result = _fetch_greenhouse_api(
            url, timeout, max_description_chars, title_min_chars, title_max_chars,
