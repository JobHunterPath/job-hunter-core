...
from job_hunter_core.core.api_budget import is_api_quota_exhausted, mark_api_exhausted
...
def _probe_remotive(
    title: str, _regions: dict[str, dict], cfg: dict, _config: dict
) -> SourceProbeResult:
    return _probe_json(
        "remotive",
        lambda: requests.get(
...
def _probe_remoteok(
    ...
    return _probe_json(
        "remoteok",
        lambda: requests.get(
            remoteok_source._API_URL, headers=remoteok_source._HEADERS, timeout=_timeout(cfg)
        ),
        allow_list=True,
    )


def _probe_weworkremotely(
    _title: str, _regions: dict[str, dict], cfg: dict, _config: dict
) -> SourceProbeResult:
    return _probe_rss(
        "weworkremotely",
...
def _source_probes() -> dict[
    str, Callable[[str, dict[str, dict], dict[str, Any], dict], SourceProbeResult]
]:
    return {
