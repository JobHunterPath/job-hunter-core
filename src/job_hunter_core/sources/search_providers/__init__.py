"""Search and career-page provider strategies.

Re-exports all public symbols from the search_providers package so that
existing import sites require no changes.
"""

from __future__ import annotations

import logging
import shutil  # noqa: F401 — exposed so tests can patch search_providers.shutil.which
import subprocess  # noqa: F401 — exposed so tests can patch search_providers.subprocess.run

import requests  # noqa: F401 — exposed so tests can patch search_providers.requests.get/post

from job_hunter_core.sources.search_providers._constants import (
    BRAVE_SUPPORTED_COUNTRIES,
    BRAVE_URL,
    EXA_URL,
    JOB_HINTS,
    TAVILY_URL,
    TRACKING_QUERY_KEYS,
    TRACKING_QUERY_PREFIXES,
    USER_AGENT,
)
from job_hunter_core.sources.search_providers._result import (
    SearchResult,
    normalize_web_results,
)
from job_hunter_core.sources.search_providers._url_utils import (
    _location_match,
    _looks_like_job_url,
    _text,
    _with_scheme,
    canonicalize_url,
)
from job_hunter_core.sources.search_providers.ats_discovery import (
    _ats_search_queries,
    _enrich_ats_discovery_job,
    discover_ats_jobs_by_search,
)
from job_hunter_core.sources.search_providers.discovery import (
    discover_company_homepage,
    search_career_urls,
)
from job_hunter_core.sources.search_providers.fetchers import (
    _jobs_from_markdown_links,
    extract_jobs_from_html,
    fetch_firecrawl_career_jobs,
    fetch_lightpanda_career_jobs,
    fetch_playwright_career_jobs,
    fetch_static_career_jobs,
)
from job_hunter_core.sources.search_providers.providers import (
    BraveProvider,
    ExaProvider,
    SearchProvider,
    SearxngProvider,
    TavilyProvider,
)
from job_hunter_core.sources.search_providers.router import (
    _PROVIDER_FAILURES,
    _SEARXNG_ZERO_LOCK,
    ProviderSearchRouter,
    SearchRouter,
    all_providers_exhausted,
    search_web,
)

__all__ = [
    "_PROVIDER_FAILURES",
    "_SEARXNG_ZERO_LOCK",
    "_ats_search_queries",
    "_enrich_ats_discovery_job",
    "logger",
    "BRAVE_SUPPORTED_COUNTRIES",
    "BRAVE_URL",
    "EXA_URL",
    "JOB_HINTS",
    "TAVILY_URL",
    "TRACKING_QUERY_KEYS",
    "TRACKING_QUERY_PREFIXES",
    "USER_AGENT",
    "SearchResult",
    "normalize_web_results",
    "_location_match",
    "_looks_like_job_url",
    "_text",
    "_with_scheme",
    "canonicalize_url",
    "discover_ats_jobs_by_search",
    "discover_company_homepage",
    "search_career_urls",
    "_jobs_from_markdown_links",
    "extract_jobs_from_html",
    "fetch_firecrawl_career_jobs",
    "fetch_lightpanda_career_jobs",
    "fetch_playwright_career_jobs",
    "fetch_static_career_jobs",
    "BraveProvider",
    "ExaProvider",
    "SearchProvider",
    "SearxngProvider",
    "TavilyProvider",
    "ProviderSearchRouter",
    "SearchRouter",
    "all_providers_exhausted",
    "search_web",
]

logger = logging.getLogger(__name__)
