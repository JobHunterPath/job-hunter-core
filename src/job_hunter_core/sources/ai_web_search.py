...
from job_hunter_core.core.config import ROOT, get_secret, load_api_config
...
def build_rule_context(
    search_config: dict[str, Any], title_filters: list[str], region_config: dict[str, Any]
) -> str:
...
def build_queries(
    title: str, region_config: dict[str, Any], config: dict[str, Any]
) -> list[tuple[str, str]]:
    location = region_config.get("location") or region_config.get("name") or ""
...
def _looks_stale(item: dict[str, Any], search_config: dict[str, Any]) -> bool:
    stale_indicators = (search_config.get("exclusion_rules", {}) or {}).get(
        "stale_indicators", []
    ) or []
    if not stale_indicators:
        return False
    combined = " ".join(
        str(item.get(key) or "").lower() for key in ("title", "snippet", "description")
    )
