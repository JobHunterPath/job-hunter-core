...
- Announce the rename in the release notes with the old name, the new name, and the migration action.

Do not silently rename a key and ship it without documentation.
...
5. Announce the removal in the release notes with a migration note for users.
...
- Set `"additionalProperties": false` on every object schema to catch typos early.
- List all required fields explicitly in `"required"`.
- Keep descriptions on every property.
