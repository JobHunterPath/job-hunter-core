...
New job result code returns `list[JobPosting]`; legacy dict callers use `JobPosting.from_dict()` at the boundary.

- Never import `JobPosting` at module level in sources — use `TYPE_CHECKING` guard to avoid circular imports.
- New sources that subclass `JobSourceAdapter` must return `list[JobPosting]` from `fetch()`.
- `JobPosting.to_dict()` and `from_dict()` provide zero-breakage interop with existing dict consumers.
