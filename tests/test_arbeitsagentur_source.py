"""Tests for sources/arbeitsagentur_source.py — all HTTP calls are mocked."""

from unittest.mock import MagicMock, patch

from job_hunter_core.sources import arbeitsagentur_source as aa


def _mock_get(json_data, status=200):
    resp = MagicMock()
    resp.status_code = status
    resp.raise_for_status = MagicMock()
    resp.json.return_value = json_data
    return resp


_ENABLED_CFG = {
    "http": {
        "job_boards": {
            "arbeitsagentur": {
                "enabled": True,
                "timeout_seconds": 10,
                "results_per_query": 25,
            }
        }
    }
}

_REGIONS = {
    "berlin": {"country": "DE", "location": "Berlin"},
}

_CONFIG = {"exclusion_rules": {"excluded_title_terms": []}}

_RESPONSE = {
    "stellenangebote": [
        {
            "titel": "Product Manager",
            "arbeitgeber": "Acme GmbH",
            "refnr": "BA-12345",
            "aktuelleVeroeffentlichungsdatum": "2026-05-01T00:00:00",
            "arbeitsort": {"ort": "Berlin", "land": "Deutschland"},
            "stellenbeschreibung": "Great role.",
        },
        {
            "titel": "Sales Manager",
            "arbeitgeber": "Other GmbH",
            "refnr": "BA-99999",
            "aktuelleVeroeffentlichungsdatum": "2026-05-02T00:00:00",
            "arbeitsort": {"ort": "Munich", "land": "Deutschland"},
            "stellenbeschreibung": "Munich role.",
        },
    ]
}


def test_returns_empty_when_disabled():
    disabled_cfg = {"http": {"job_boards": {"arbeitsagentur": {"enabled": False}}}}
    with patch(
        "job_hunter_core.sources.arbeitsagentur_source.load_api_config", return_value=disabled_cfg
    ):
        jobs = aa.fetch_arbeitsagentur_jobs(["Product Manager"], _REGIONS, _CONFIG)
    assert jobs == []


def test_skips_non_de_regions():
    non_de_regions = {"london": {"country": "GB", "location": "London"}}
    with patch(
        "job_hunter_core.sources.arbeitsagentur_source.load_api_config", return_value=_ENABLED_CFG
    ):
        jobs = aa.fetch_arbeitsagentur_jobs(["Product Manager"], non_de_regions, _CONFIG)
    assert jobs == []


def test_returns_matching_jobs():
    with (
        patch(
            "job_hunter_core.sources.arbeitsagentur_source.load_api_config",
            return_value=_ENABLED_CFG,
        ),
        patch(
            "job_hunter_core.sources.arbeitsagentur_source.requests.get",
            return_value=_mock_get(_RESPONSE),
        ),
    ):
        jobs = aa.fetch_arbeitsagentur_jobs(["Product Manager"], _REGIONS, _CONFIG)
    assert len(jobs) == 1
    job = jobs[0]
    assert job["title"] == "Product Manager"
    assert job["company"] == "Acme GmbH"
    assert job["source"] == "Arbeitsagentur"
    assert "BA-12345" in job["url"]
    assert job["posted"] == "2026-05-01"
    assert job["region"] == "berlin"


def test_returns_empty_on_api_error():
    with (
        patch(
            "job_hunter_core.sources.arbeitsagentur_source.load_api_config",
            return_value=_ENABLED_CFG,
        ),
        patch(
            "job_hunter_core.sources.arbeitsagentur_source.requests.get",
            side_effect=Exception("timeout"),
        ),
    ):
        jobs = aa.fetch_arbeitsagentur_jobs(["Product Manager"], _REGIONS, _CONFIG)
    assert jobs == []


def test_empty_response_returns_empty():
    with (
        patch(
            "job_hunter_core.sources.arbeitsagentur_source.load_api_config",
            return_value=_ENABLED_CFG,
        ),
        patch(
            "job_hunter_core.sources.arbeitsagentur_source.requests.get",
            return_value=_mock_get({"stellenangebote": []}),
        ),
    ):
        jobs = aa.fetch_arbeitsagentur_jobs(["Product Manager"], _REGIONS, _CONFIG)
    assert jobs == []


class TestArbeitsagenturSource:
    def test_name(self):
        assert aa.ArbeitsagenturSource().name == "arbeitsagentur"

    def test_is_enabled_false_when_disabled(self):
        disabled = {"http": {"job_boards": {"arbeitsagentur": {"enabled": False}}}}
        with patch(
            "job_hunter_core.sources.arbeitsagentur_source.load_api_config", return_value=disabled
        ):
            assert aa.ArbeitsagenturSource().is_enabled({}) is False

    def test_fetch_returns_job_postings(self):
        from job_hunter_core.models import JobPosting

        with (
            patch(
                "job_hunter_core.sources.arbeitsagentur_source.load_api_config",
                return_value=_ENABLED_CFG,
            ),
            patch(
                "job_hunter_core.sources.arbeitsagentur_source.requests.get",
                return_value=_mock_get(_RESPONSE),
            ),
        ):
            jobs = aa.ArbeitsagenturSource().fetch(["Product Manager"], _REGIONS, _CONFIG)
        assert len(jobs) >= 1
        assert isinstance(jobs[0], JobPosting)
        assert jobs[0].source == "Arbeitsagentur"
