...
from job_hunter_core.sources import career_pages
...
def test_detect_ats_unknown_returns_empty():
...
def test_extract_jsonld_jobs_finds_single_posting():
    jobs = career_pages.extract_jsonld_jobs(
        _JSONLD_HTML, "https://careers.testcorp.com", "TestCorp"
    )
    assert len(jobs) == 1
    job = jobs[0]
...
def test_extract_jsonld_jobs_finds_list_of_postings():
    jobs = career_pages.extract_jsonld_jobs(
        _JSONLD_LIST_HTML, "https://careers.testcorp.com", "TestCorp"
    )
    assert len(jobs) == 2
...
def test_extract_jsonld_jobs_returns_empty_when_none_present():
    jobs = career_pages.extract_jsonld_jobs(
        _NO_JSONLD_HTML, "https://careers.testcorp.com", "TestCorp"
    )
    assert jobs == []


def test_extract_jsonld_jobs_deduplicates_same_apply_url():
    html = """
    <html>
    <head>
...
def test_extract_jsonld_jobs_preserves_raw_schema_org():
    jobs = career_pages.extract_jsonld_jobs(
        _JSONLD_HTML, "https://careers.testcorp.com", "TestCorp"
    )
...
def test_extract_career_page_jobs_uses_ats_api_for_greenhouse_url():
    ...
    with patch("job_hunter_core.sources.career_pages.requests.get", return_value=mock_resp):
...
def test_extract_career_page_jobs_falls_through_to_jsonld_when_ats_api_empty():
    ...
    with patch("job_hunter_core.sources.career_pages.requests.get", side_effect=fake_get):
        jobs = career_pages.extract_career_page_jobs(
            company,
...
def test_extract_career_page_jobs_no_search_provider_called():
    ...
    with (
        ...
        patch("job_hunter_core.sources.career_pages.requests.head", return_value=mock_head),
        patch("job_hunter_core.sources.search_providers.SearchRouter.search") as mock_search,
    ):
...
def test_extract_career_page_jobs_uses_lightpanda_before_playwright():
    ...
    with (
        ...
        patch(
            "job_hunter_core.sources.career_pages._ladder.extract_from_lightpanda",
            return_value=[{"title": "Product Manager", "url": "https://example.com/jobs/1"}],
        ),
        patch("job_hunter_core.sources.career_pages._ladder.extract_from_rendered_html") as mock_pw,
    ):
