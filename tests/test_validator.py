...
from job_hunter_core.pipeline import validator
...
def test_validate_accepts_fenced_json_with_preamble():
    ...
    with patch("job_hunter_core.pipeline.validator.get_llm_client", return_value=_mock_client(raw)):
...
def test_validate_uses_injected_url_checker_before_llm():
    ...
    with patch("job_hunter_core.pipeline.validator.get_llm_client") as client:
...
def test_validate_sends_ambiguous_experience_to_llm():
    ...
    with patch(
        "job_hunter_core.pipeline.validator.get_llm_client", return_value=_mock_client(raw)
    ) as client:
        valid, rejected = validator.validate(jobs, max_years=4, api_cfg=api_cfg)
...
def test_validate_requests_json_response_format():
    ...
    with patch("job_hunter_core.pipeline.validator.get_llm_client", return_value=mock):
        valid, rejected = validator.validate(jobs, max_years=4, api_cfg=api_cfg)
...
def test_validate_strategic_company_bypasses_deterministic_years_rejection():
    jobs = [
        {
            "title": "Product Manager",
