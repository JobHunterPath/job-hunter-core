...
from job_hunter_core.pipeline import cover_writer
...
def test_write_cover_creates_markdown(tmp_path):
    with patch(
        "job_hunter_core.pipeline.cover_writer.get_llm_client", return_value=_mock_client(BODY)
    ):
...
def test_write_cover_api_error_raises(tmp_path):
    ...
    with patch("job_hunter_core.pipeline.cover_writer.get_llm_client", return_value=mock):
...
def test_write_cover_no_story_id_citations(tmp_path):
    ...
    with patch(
        "job_hunter_core.pipeline.cover_writer.get_llm_client",
        return_value=_mock_client(body_with_citation),
    ):
        md_path = cover_writer.write_cover(MATCH, str(tmp_path), CONFIG)
