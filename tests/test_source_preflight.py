...
from job_hunter_core.sources import scraper as scraper_module
...
def test_scrape_skips_disabled_source(monkeypatch: pytest.MonkeyPatch) -> None:
    disabled = {
        name: preflight.SourceProbeResult(name, "disabled", "test")
        for name in preflight._source_probes()
    }
    ...
    monkeypatch.setattr(scraper_module, "discover_ats_jobs_by_search", lambda *args, **kwargs: [])
    monkeypatch.setattr(
        scraper_module.JobicySource,
        "fetch",
