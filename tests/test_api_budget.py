"""Tests for job_hunter_core.core.api_budget."""
...
def test_monthly_reset_clears_state(tmp_path: Path) -> None:
    ...
    old_state = {
        "month": "2000-01",
        "providers": {"brave": 999},
        "exhausted": {"brave": {"reason": "old", "marked_on": "2000-01-01"}},
    }
    ...
    assert "brave" not in data.get("exhausted", {})
...
def test_mark_exhausted_writes_reason(tmp_path: Path) -> None:
    ...
    budget.mark_api_exhausted(
        "adzuna", reason="402 Payment Required", api_cfg={"http": {"api_budgets": cfg}}
    )
    state_file = Path(cfg["state_path"])
    data = json.loads(state_file.read_text(encoding="utf-8"))
