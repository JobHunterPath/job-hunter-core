...
from job_hunter_core.core import llm_client
from job_hunter_core.core.llm_client import LLMClient, _google_generation_config_kwargs
...
def test_throttle_noops_when_disabled(monkeypatch):
    ...
    sleep = MagicMock()
    monkeypatch.setattr("job_hunter_core.core.llm_client.time.sleep", sleep)
...
def test_throttle_waits_when_window_is_full(monkeypatch):
    ...
    monkeypatch.setattr("job_hunter_core.core.llm_client.time.monotonic", lambda: next(times))
    monkeypatch.setattr("job_hunter_core.core.llm_client.time.sleep", sleep)
...
def test_get_llm_client_cache_is_thread_safe(monkeypatch):
    ...
    monkeypatch.setattr("job_hunter_core.core.config.load_api_config", lambda: config)
    monkeypatch.setattr("job_hunter_core.core.config.get_secret", lambda *args, **kwargs: "")
