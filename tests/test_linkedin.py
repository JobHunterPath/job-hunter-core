...
from job_hunter_core.linkedin import common, discover_engagement, draft_posts, generate_ideas
...
def test_linkedin_yaml_files_parse():
    files = [
        ...
        "linkedin/config.yml",
        "profile/template-files/linkedin/config.yml",
...
def test_template_linkedin_is_disabled_by_default():
    config = yaml.safe_load(
        (Path(__file__).parent.parent / "profile/template-files/linkedin/config.yml").read_text(
            encoding="utf-8"
        )
    )["linkedin"]

    assert config["enabled"] is False
    assert "enabled" not in config["idea_generation"]
    assert "enabled" not in config["draft_generation"]
    assert "recruiter_discovery" not in config
    assert "discovery_policy" not in config
    assert "dedupe" not in config
    assert "quality" not in config
    assert "llm_caps" not in config
    assert "target_people" not in config["networking"]


def test_linkedin_internal_defaults_are_packaged_and_parse():
    with (
        resources.files("job_hunter_core.linkedin")
        .joinpath("defaults.yml")
        .open(encoding="utf-8") as defaults
    ):
...
def test_disabled_linkedin_discovery_exits_without_search_or_llm(tmp_path):
    ...
    with (
        patch("job_hunter_core.linkedin.discover_engagement.search_web") as search,
        patch("job_hunter_core.linkedin.discover_engagement.complete_linkedin") as llm,
    ):
...
def test_generate_ideas_appends_public_safe_items(tmp_path):
    ...
    with patch("job_hunter_core.linkedin.generate_ideas.complete_linkedin", return_value=payload):
...
def test_generate_ideas_prompt_does_not_assume_pm_or_po(tmp_path):
    cfg = _config(tmp_path)
    data = yaml.safe_load(cfg.read_text(encoding="utf-8"))
    ...
    with patch(
        "job_hunter_core.linkedin.generate_ideas.complete_linkedin", side_effect=fake_complete
    ):
...
""",
    ...
    with patch("job_hunter_core.linkedin.draft_posts.complete_linkedin", return_value=payload):
...
def test_discover_networking_writes_networking_queue(tmp_path):
    cfg = _config(tmp_path)
    search_result = [
        {
    ...
    with (
        patch(
            "job_hunter_core.linkedin.discover_engagement.search_web", return_value=search_result
        ),
        patch(
            "job_hunter_core.linkedin.discover_engagement.complete_linkedin",
...
def test_discover_networking_falls_back_on_malformed_json(tmp_path):
    cfg = _config(tmp_path)
    search_result = [
        {
            "url": "https://www.linkedin.com/in/example",
...
def test_search_strategy_uses_existing_search_config_context(tmp_path):
    cfg = _config(tmp_path)
    captured = {}
    ...
    with patch(
        "job_hunter_core.linkedin.discover_engagement._search_context", return_value=fake_context
    ):
        with patch(
            "job_hunter_core.linkedin.discover_engagement.complete_linkedin",
            ...
            strategy = discover_engagement._search_strategy(
                yaml.safe_load(cfg.read_text(encoding="utf-8"))["linkedin"]
            )

    assert "TARGET JOB TITLES FROM SEARCH CONFIG" in captured["prompt"]
    assert "Product Manager" in captured["prompt"]
    assert strategy["people_queries"]


def test_discovery_defaults_can_follow_non_pm_profiles(tmp_path):
    cfg = _config(tmp_path)
    data = yaml.safe_load(cfg.read_text(encoding="utf-8"))
    data["linkedin"]["positioning"] = "Cloud infrastructure engineer focused on reliability."
    cfg.write_text(yaml.safe_dump(data), encoding="utf-8")
...
def test_discovery_dedupes_seen_people(tmp_path):
    cfg = _config(tmp_path)
    state = {
...
def test_recruiters_are_prioritized_with_specific_messages(tmp_path):
    cfg = _config(tmp_path)
    search_result = [
        {
...
def test_llm_caps_limit_generation_candidates(tmp_path):
    cfg = _config(tmp_path)
    search_result = [
        {
