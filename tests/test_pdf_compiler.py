...
from job_hunter_core.pipeline import pdf_compiler
...
def test_compile_tex_returns_pdf_path_when_pdflatex_succeeds(tmp_path):
    ...
    with (
        patch(
            "job_hunter_core.pipeline.pdf_compiler.shutil.which", return_value="/usr/bin/pdflatex"
        ),
        patch(
            "job_hunter_core.pipeline.pdf_compiler.subprocess.run",
...
def test_compile_tex_returns_none_when_no_pdf_produced(tmp_path):
    ...
    with (
        ...
        patch("job_hunter_core.pipeline.pdf_compiler.subprocess.run", return_value=mock_result),
    ):
        result = pdf_compiler.compile_tex(str(tex), str(tmp_path))
...
def test_compile_tex_uses_docker_when_no_pdflatex(tmp_path):
    ...
    with (
        patch("job_hunter_core.pipeline.pdf_compiler.shutil.which", return_value=None),
        patch("job_hunter_core.pipeline.pdf_compiler.subprocess.run", side_effect=_run),
    ):
        result = pdf_compiler.compile_tex(str(tex), str(tmp_path))
